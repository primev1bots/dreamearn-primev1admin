...
const TelegramNotifier: React.FC = () => {
  ...
  useEffect(() => {
    const checkBackendStatus = async () => {
      try {
        const response = await fetch("https://dreamearn-primev1-server.onrender.com/api/health");
        if (response.ok) {
  ...
  const testBotToken = async (token: string): Promise<boolean> => {
    try {
      const response = await fetch("https://dreamearn-primev1-server.onrender.com/api/test-notification", {
        method: "POST",
  ...
  const sendNotification = async () => {
    ...
    try {
      ...
      const res = await fetch("https://dreamearn-primev1-server.onrender.com/api/send-notification", {
        method: "POST",
        headers: { 
